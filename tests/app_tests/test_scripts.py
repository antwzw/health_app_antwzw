...
def test_healthdb_destroy():
    """Verify healthdb destroy removes DB file."""
    assert_is_shell_script("bin/healthdb")
    subprocess.run(["bin/healthdb", "destroy"], check=True)
    assert not os.path.exists("var/health.sqlite3")
    assert not os.path.exists("var/uploads")


def test_healthdb_create():
    """Verify healthdb create populates DB with default data."""
    assert_is_shell_script("bin/healthdb")

    # Destroy, then create database
    subprocess.run(["bin/healthdb", "destroy"], check=True)
    subprocess.run(["bin/healthdb", "create"], check=True)

    # Verify files were created
    assert os.path.exists("var/health.sqlite3")
    ...
    connection = sqlite3.connect("var/health.sqlite3")
...
def test_healthdb_reset():
    """Verify healthdb reset does a destroy and a create."""
    # Create a "stale" database file
    dbfile = pathlib.Path("var/health.sqlite3")
    dbfile.write_text("this should be overwritten")

    # Reset the database
    subprocess.run(["bin/healthdb", "reset"], check=True)
...
def test_healthdb_dump():
    """Spot check healthdb dump for a few data points."""
    assert_is_shell_script("bin/healthdb")
    subprocess.run(["bin/healthdb", "reset"], check=True)
...
def test_healthrun(setup_teardown):
    """Verify healthrun script behavior."""
    ...
    db_path = pathlib.Path("var/health.sqlite3")
    if db_path.exists():
        db_path.unlink()
    completed_process = subprocess.run(["bin/healthrun"])
    assert completed_process.returncode != 0

    # Create database
    completed_process = subprocess.run(["bin/healthdb", "create"])
    assert os.path.exists("var/health.sqlite3")

    # Execute student run script in a concurrent thread.  Don't check the
    # return code because test cleanup will kill the process
    assert_is_shell_script("bin/healthrun")
...
def test_healthtest():
    """Verify healthtest script contains correct commands."""
    assert_is_shell_script("bin/healthtest")
    lines = pathlib.Path("bin/healthtest")\
